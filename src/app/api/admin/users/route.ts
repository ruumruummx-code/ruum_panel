...
async function getActor(){
  ...
  if(!user) return null;
  // Check superadmin via profiles or fallback email
  const { data: profile } = await supabase.from("profiles").select("role").eq("id", user.id).maybeSingle();
  const role = (profile as any)?.role || (user.email === "lomelinhectorm@gmail.com" ? "superadmin" : null);
...
export async function POST(request: Request){
  ...
  // Insert profile (si existe tabla)
  ...
  // Si profiles no existe, no bloquea creación del auth user
