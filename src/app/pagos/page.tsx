    ...
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-xs text-slate-500 border-y">
              <tr><th className="text-left px-4 py-2">traslados</th><th className="text-left px-4 py-2">Cliente</th><th className="text-right px-4 py-2">Tarifa</th><th className="text-left px-4 py-2">Método</th><th className="text-left px-4 py-2">Estatus</th><th className="text-left px-4 py-2">Fecha</th></tr>
            </thead>
            <tbody className="divide-y">
              {pagos.map(p=>(
                <tr key={p.id} className="hover:bg-slate-50"><td className="px-4 py-3 font-mono font-medium">{p.traslados}</td><td className="px-4 py-3">{p.cliente}</td><td className="px-4 py-3 text-right font-semibold">{money(p.tarifa)}</td><td className="px-4 py-3">{p.metodo}</td><td className="px-4 py-3"><Badge variant={p.estatus==="Pagado"?"success":p.estatus==="Pendiente"?"warning":"neutral"}>{p.estatus}</Badge></td><td className="px-4 py-3 text-slate-500">{p.fecha}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="mt-3 space-y-2 text-sm">
            {[
              ...
              <div key={r.n} className="flex items-center justify-between rounded-xl border p-3">
                <div><div className="font-medium">{r.n}</div><div className="text-xs text-slate-500">{r.v} traslados</div></div>
        ...
        <Card className="p-5">
          ...
          <div className="mt-3 space-y-2 text-sm">
            {[
              {t:"Peaje", traslados:"RR-24081", monto:450, est:"Aprobado"},
              {t:"Combustible", traslados:"RR-24084", monto:800, est:"En revisión"},
              {t:"Peaje", traslados:"RR-24085", monto:320, est:"Rechazado"},
            ].map(g=>(
              <div key={g.traslados} className="flex items-center justify-between rounded-xl border p-3">
                <div><div className="font-medium">{g.t} • {g.traslados}</div><div className="text-xs text-slate-500">{money(g.monto)}</div></div>
