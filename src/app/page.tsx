import { kpis, traslados, incidencias } from "@/data/mock";
...
const kpiCards = [
  { label:"traslados activos", value:kpis.activos, sub:"+2 vs ayer", icon:Truck, color:"bg-[#ff4d11]" },
  { label:"Pendientes de asignación", value:kpis.pendientes, sub:"Requieren atención", icon:Clock, color:"bg-amber-500" },
  { label:"Conductores disponibles", value:kpis.conductoresDisponibles, sub:`${kpis.conductoresEntraslados} en traslados`, icon:Users, color:"bg-emerald-500" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Link href="/traslados" className="inline-flex items-center gap-2 bg-slate-900 text-white rounded-xl px-4 h-9 text-sm font-medium">Ver todos los traslados <ArrowUpRight className="w-4 h-4"/></Link>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="flex flex-col">
          ...
          <div className="px-3 space-y-2 flex-1">
            {[
              {t:"traslados sin conductor", d:"RR-24082 • Ana Torres • hace 42 min", c:"warning"},
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card>
          <div className="p-5 pb-3 flex items-center justify-between"><h3 className="font-semibold flex items-center gap-2"><Clock className="w-4 h-4"/> Próximos traslados</h3><Link href="/traslados" className="text-xs font-medium text-[#ff4d11]">Ver todos</Link></div>
          <div className="px-3 pb-3 space-y-2">
            {traslados.slice(0,4).map(v=>(
              <Link key={v.id} href={`/traslados/${v.id}`} className="flex gap-3 p-3 rounded-xl border hover:bg-slate-50">
        ...
        <Card>
          ...
          <div className="px-5 pb-5 space-y-4">
            <div className="rounded-xl bg-slate-900 text-white p-4">
              ...
              <div className="text-xs opacity-60 mt-1">5 traslados finalizados + 8 activos</div>
            </div>
