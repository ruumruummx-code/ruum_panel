        ...
        {f.map(c=>(
          <Card key={c.id} className="p-5">
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-1.5 mt-1.5 flex-wrap">
                  <Badge variant={c.disp==="Disponible"?"success":c.disp==="En viaje"?"warning":"neutral"}>{c.disp}</Badge>
                  <Badge variant={c.cert==="Activo"?"success":c.cert==="Vencido"?"danger":"warning"}>{c.cert}</Badge>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2 mt-4 text-center">
              <div className="rounded-xl border bg-slate-50 p-2"><div className="font-bold">{c.viajes}</div><div className="text-[11px] text-slate-500">Viajes</div></div>
            ...
            <div className="flex gap-2 mt-4">
              ...
              <Button size="sm" className="flex-1">Asignar viaje</Button>
            </div>
          </Card>
        ))}
      </div>
